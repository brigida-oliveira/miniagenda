...
def delete_categoria(request, id_categoria):
    ...
    if categoria.user == request.user:
        ...
        return render(request, 'tarefas/lista_categoria.html')
    return redirect('tarefas:lista_categorias')

@login_required
def editar_categoria(request, id_categoria):
    categoria = get_object_or_404(Categoria, id=id_categoria, user=request.user) #mesma coisa que Categoria.objects.get(id=id_tarefa)
    if request.method == 'POST':
...
def delete_tarefa(request, id_tarefa):
    ...
    if tarefa.user == request.user:
        ...
        return render('core/intex.html')
    return redirect('core')

@login_required
def editar_tarefa(request, id_tarefa):
    tarefa = get_object_or_404(Tarefa, id=id_tarefa, user=request.user) #mesma coisa que Tarefa.objects.get(id=id_tarefa)
    if request.method == 'POST':
...
def search(request):
    ...
    return render(request, 'tarefas/resultado.html', {'result':result})
